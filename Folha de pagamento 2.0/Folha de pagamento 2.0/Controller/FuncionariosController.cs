...
namespace Folha_de_pagamento_2._0
{
    public class FuncionariosController
    {
        ...
        {
            strsql = @"update funcionario set CPF = @CPF, Nome = @Nome, Endereço = @Endereço, Bairro = @Bairro, CEP=@CEP, Telefone=@Telefone, UF=@UF, Cidade=@Cidade where CPF=@CPF";
            conn = new SqlConnection(sql);
            ...
            strsql = "update dadostrabalhista set Cargo=@Cargo, Salariobase=@Salariobase, Horasdetrabalho=@Horasdetrabalho, Insalubridade= @Insalubridade, Pis= @Pis, Periculosidade=@Periculosidade, Dataadmissao=@Dataadmissao, Datademissao=@Datademissao, CpfFunc=@CpfFunc where CpfFunc=@CpfFunc";
            SqlCommand cmd2 = new SqlCommand(strsql, conn);
            cmd2.Parameters.Add("@Cargo", SqlDbType.VarChar).Value = funcionarios.cargo;
            cmd2.Parameters.Add("@Salariobase", SqlDbType.Decimal).Value = funcionarios.salariobase;
            cmd2.Parameters.Add("@Horasdetrabalho", SqlDbType.Int).Value = funcionarios.horastrabalho;
